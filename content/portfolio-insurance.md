...
### ทดสอบกับ S&P 500 ปี 2018–2025: CPPI เทียบ Buy & Hold

กราฟเริ่มที่ 100 หน่วย ณ ราคาปิด 29 ธันวาคม 2017 แล้วทบผลรายปีต่อกันถึง 31 ธันวาคม 2025 **ตั้ง Floor ใหม่ทุกต้นปีที่ 90% ของเงินที่มีตอนนั้น** ใช้ราคาปิดรายวัน ดอกเบี้ย 0% ไม่รวมปันผลและต้นทุน ตาม[กติกาทดสอบ S&P 500](#sp500-test) เส้นประคือ Buy & Hold; เส้นทึบคือ CPPI ที่ใช้ $m=3$ และปรับพอร์ตรายวัน

<figure class="lesson-figure sp500-figure" id="sp500-cppi">
<picture>
<source media="(max-width: 600px)" srcset="assets/charts/sp500-cppi-mobile.svg">
<img src="assets/charts/sp500-cppi.svg" alt="กราฟเส้น CPPI เทียบ Buy &amp; Hold รายวัน ปี 2018–2025 เริ่ม 100 จบที่ 136.33 และ 256.04 หน่วยตามลำดับ ตั้ง Floor ใหม่ทุกปี" width="800" height="460">
</picture>
<figcaption>CPPI จบที่ 136.33 หน่วย เทียบ Buy &amp; Hold 256.04 หน่วย จากเงินเริ่ม 100 เท่ากัน กราฟใช้ 2,012 จุดรวมจุดเริ่มต้น แกนตั้งและช่วงเวลาเท่ากันทั้งสี่วิธี ที่มา: S&amp;P Dow Jones Indices ผ่าน FRED SP500; คำนวณตามกฎของบทเรียน</figcaption>
</figure>

**Drawdown ระหว่างทาง:** คำนวณจาก `(มูลค่าพอร์ต ÷ จุดสูงสุดสะสมของพอร์ต − 1) × 100` เส้นที่ 0% คืออยู่ที่จุดสูงสุด; −10% คือต่ำกว่าจุดสูงสุด 10% แต่ละเส้นใช้จุดสูงสุดของพอร์ตตัวเอง รวมเงินเริ่มต้น 100 และ **ไม่เริ่มวัดใหม่ต้นปี** แม้ Floor ของกลยุทธ์จะเริ่มรอบใหม่ก็ตาม

<figure class="lesson-figure sp500-figure sp500-drawdown" id="sp500-cppi-drawdown">
<picture>
<source media="(max-width: 600px)" srcset="assets/charts/sp500-cppi-drawdown-mobile.svg">
<img src="assets/charts/sp500-cppi-drawdown.svg" alt="กราฟ Drawdown รายวัน 2018–2025 ของ CPPI เทียบ Buy &amp; Hold จุดต่ำสุด −8.66% และ −33.92% ตามลำดับ วัดจากจุดสูงสุดของแต่ละพอร์ตต่อเนื่องข้ามปี" width="800" height="460">
</picture>
<figcaption>Drawdown ของ CPPI เทียบ Buy &amp; Hold จากเส้นทางเดียวกับกราฟมูลค่าด้านบน ใช้สเกล 0% ถึง −40% เหมือนกันทั้งสี่วิธี จุดต่ำสุดของเส้นเท่ากับ −8.66% และ −33.92% ตามลำดับ; Maximum drawdown รายงานเป็นขนาดบวก 8.66% และ 33.92% ไม่ใช่อัตรารายปี ที่มา: S&amp;P Dow Jones Indices ผ่าน FRED SP500; คำนวณตามกฎของบทเรียน</figcaption>
</figure>

ปี 2020 เส้น CPPI ลดลงน้อยกว่า Buy & Hold แต่เมื่อดัชนีฟื้น เงินที่กันไว้ทำให้พอร์ตรับขาขึ้นได้น้อยกว่าเช่นกัน การตั้ง Floor ใหม่ทุกปีในกราฟนี้หมายถึง 90% ของมูลค่าปลายปีก่อน ไม่ใช่รักษา Floor 90 หน่วยเดิมตลอดแปดปี

Maximum drawdown **ตลอดเส้นทางแปดปีนี้** เท่ากับ **8.66%** สำหรับ CPPI เทียบ **33.92%** ของ Buy & Hold ค่านี้วัดจากจุดสูงสุดก่อนหน้าโดยไม่เริ่มนับใหม่ต้นปี จึงต่างจากการเลือก Drawdown ที่มากที่สุดจากตารางการทดลองรายปี
...
#### ทดสอบกับ S&P 500 ปี 2018–2025: Variable m เทียบ Buy & Hold

กราฟเริ่มที่ 100 หน่วย ณ ราคาปิด 29 ธันวาคม 2017 แล้วทบผลรายปีต่อกันถึง 31 ธันวาคม 2025 **ตั้ง Floor ใหม่ทุกต้นปีที่ 90% ของเงินที่มีตอนนั้น** ใช้ราคาปิดรายวัน ดอกเบี้ย 0% ไม่รวมปันผลและต้นทุน ตาม[กติกาทดสอบ S&P 500](#sp500-test) เส้นประคือ Buy & Hold; เส้นทึบคือ Variable m

<figure class="lesson-figure sp500-figure" id="sp500-variable-m">
<picture>
<source media="(max-width: 600px)" srcset="assets/charts/sp500-variable-m-mobile.svg">
<img src="assets/charts/sp500-variable-m.svg" alt="กราฟเส้น Variable m เทียบ Buy &amp; Hold รายวัน ปี 2018–2025 เริ่ม 100 จบที่ 155.22 และ 256.04 หน่วยตามลำดับ ตั้ง Floor ใหม่ทุกปี" width="800" height="460">
</picture>
<figcaption>Variable m จบที่ 155.22 หน่วย เทียบ Buy &amp; Hold 256.04 หน่วย จากเงินเริ่ม 100 เท่ากัน กราฟใช้ 2,012 จุดรวมจุดเริ่มต้น แกนตั้งและช่วงเวลาเท่ากันทั้งสี่วิธี ที่มา: S&amp;P Dow Jones Indices ผ่าน FRED SP500; คำนวณตามกฎของบทเรียน</figcaption>
</figure>

**Drawdown ระหว่างทาง:** คำนวณจาก `(มูลค่าพอร์ต ÷ จุดสูงสุดสะสมของพอร์ต − 1) × 100` เส้นที่ 0% คืออยู่ที่จุดสูงสุด; −10% คือต่ำกว่าจุดสูงสุด 10% แต่ละเส้นใช้จุดสูงสุดของพอร์ตตัวเอง รวมเงินเริ่มต้น 100 และ **ไม่เริ่มวัดใหม่ต้นปี** แม้ Floor ของกลยุทธ์จะเริ่มรอบใหม่ก็ตาม

<figure class="lesson-figure sp500-figure sp500-drawdown" id="sp500-variable-m-drawdown">
<picture>
<source media="(max-width: 600px)" srcset="assets/charts/sp500-variable-m-drawdown-mobile.svg">
<img src="assets/charts/sp500-variable-m-drawdown.svg" alt="กราฟ Drawdown รายวัน 2018–2025 ของ Variable m เทียบ Buy &amp; Hold จุดต่ำสุด −9.08% และ −33.92% ตามลำดับ วัดจากจุดสูงสุดของแต่ละพอร์ตต่อเนื่องข้ามปี" width="800" height="460">
</picture>
<figcaption>Drawdown ของ Variable m เทียบ Buy &amp; Hold จากเส้นทางเดียวกับกราฟมูลค่าด้านบน ใช้สเกล 0% ถึง −40% เหมือนกันทั้งสี่วิธี จุดต่ำสุดของเส้นเท่ากับ −9.08% และ −33.92% ตามลำดับ; Maximum drawdown รายงานเป็นขนาดบวก 9.08% และ 33.92% ไม่ใช่อัตรารายปี ที่มา: S&amp;P Dow Jones Indices ผ่าน FRED SP500; คำนวณตามกฎของบทเรียน</figcaption>
</figure>

เส้น Variable m ใช้ตัวคูณจากความผันผวนย้อนหลังตามสูตรที่เพิ่งอธิบาย ช่วงนี้จบสูงกว่า CPPI ที่ใช้ตัวคูณคงที่ แต่ Drawdown สูงกว่า และไม่ได้ชนะทุกปี: ปี 2018 กับ 2025 ให้ผลตอบแทนน้อยกว่า CPPI การปรับตัวคูณจึงเพิ่มทั้งโอกาสและความเสี่ยงจากการประมาณค่าความผันผวน

Maximum drawdown **ตลอดเส้นทางแปดปีนี้** เท่ากับ **9.08%** สำหรับ Variable m เทียบ **33.92%** ของ Buy & Hold ค่านี้วัดจากจุดสูงสุดก่อนหน้าโดยไม่เริ่มนับใหม่ต้นปี จึงต่างจากการเลือก Drawdown ที่มากที่สุดจากตารางการทดลองรายปี
...
### ทดสอบกับ S&P 500 ปี 2018–2025: TIPP เทียบ Buy & Hold

กราฟเริ่มที่ 100 หน่วย ณ ราคาปิด 29 ธันวาคม 2017 แล้วทบผลรายปีต่อกันถึง 31 ธันวาคม 2025 **ตั้ง Floor ใหม่ทุกต้นปีที่ 90% ของเงินที่มีตอนนั้น** ใช้ราคาปิดรายวัน ดอกเบี้ย 0% ไม่รวมปันผลและต้นทุน ตาม[กติกาทดสอบ S&P 500](#sp500-test) เส้นประคือ Buy & Hold; เส้นทึบคือ TIPP ที่ใช้ $m=3$ และปรับพอร์ตรายวัน

<figure class="lesson-figure sp500-figure" id="sp500-tipp">
<picture>
<source media="(max-width: 600px)" srcset="assets/charts/sp500-tipp-mobile.svg">
<img src="assets/charts/sp500-tipp.svg" alt="กราฟเส้น TIPP เทียบ Buy &amp; Hold รายวัน ปี 2018–2025 เริ่ม 100 จบที่ 125.54 และ 256.04 หน่วยตามลำดับ ตั้ง Floor ใหม่ทุกปี" width="800" height="460">
</picture>
<figcaption>TIPP จบที่ 125.54 หน่วย เทียบ Buy &amp; Hold 256.04 หน่วย จากเงินเริ่ม 100 เท่ากัน กราฟใช้ 2,012 จุดรวมจุดเริ่มต้น แกนตั้งและช่วงเวลาเท่ากันทั้งสี่วิธี ที่มา: S&amp;P Dow Jones Indices ผ่าน FRED SP500; คำนวณตามกฎของบทเรียน</figcaption>
</figure>

**Drawdown ระหว่างทาง:** คำนวณจาก `(มูลค่าพอร์ต ÷ จุดสูงสุดสะสมของพอร์ต − 1) × 100` เส้นที่ 0% คืออยู่ที่จุดสูงสุด; −10% คือต่ำกว่าจุดสูงสุด 10% แต่ละเส้นใช้จุดสูงสุดของพอร์ตตัวเอง รวมเงินเริ่มต้น 100 และ **ไม่เริ่มวัดใหม่ต้นปี** แม้ Floor ของกลยุทธ์จะเริ่มรอบใหม่ก็ตาม

<figure class="lesson-figure sp500-figure sp500-drawdown" id="sp500-tipp-drawdown">
<picture>
<source media="(max-width: 600px)" srcset="assets/charts/sp500-tipp-drawdown-mobile.svg">
<img src="assets/charts/sp500-tipp-drawdown.svg" alt="กราฟ Drawdown รายวัน 2018–2025 ของ TIPP เทียบ Buy &amp; Hold จุดต่ำสุด −7.67% และ −33.92% ตามลำดับ วัดจากจุดสูงสุดของแต่ละพอร์ตต่อเนื่องข้ามปี" width="800" height="460">
</picture>
<figcaption>Drawdown ของ TIPP เทียบ Buy &amp; Hold จากเส้นทางเดียวกับกราฟมูลค่าด้านบน ใช้สเกล 0% ถึง −40% เหมือนกันทั้งสี่วิธี จุดต่ำสุดของเส้นเท่ากับ −7.67% และ −33.92% ตามลำดับ; Maximum drawdown รายงานเป็นขนาดบวก 7.67% และ 33.92% ไม่ใช่อัตรารายปี ที่มา: S&amp;P Dow Jones Indices ผ่าน FRED SP500; คำนวณตามกฎของบทเรียน</figcaption>
</figure>

ภายในแต่ละปี TIPP ยก Floor ตามจุดสูงสุดของพอร์ต ทำให้เส้นมูลค่าเปลี่ยนแปลงน้อยลง แต่รับการเติบโตน้อยลงด้วย **จุดสูงสุดและ Floor เริ่มรอบใหม่ทุกปี** กราฟนี้จึงไม่ใช่ TIPP ที่รักษาจุดสูงสุดเดิมตลอดแปดปี; Floor ตอนเริ่มปีใหม่อาจต่ำกว่า Floor ที่ยกไว้ในปีก่อนได้

Maximum drawdown **ตลอดเส้นทางแปดปีนี้** เท่ากับ **7.67%** สำหรับ TIPP เทียบ **33.92%** ของ Buy & Hold ค่านี้วัดจากจุดสูงสุดก่อนหน้าโดยไม่เริ่มนับใหม่ต้นปี จึงต่างจากการเลือก Drawdown ที่มากที่สุดจากตารางการทดลองรายปี
...
### ทดสอบกับ S&P 500 ปี 2018–2025: SLPI เทียบ Buy & Hold

กราฟเริ่มที่ 100 หน่วย ณ ราคาปิด 29 ธันวาคม 2017 แล้วทบผลรายปีต่อกันถึง 31 ธันวาคม 2025 **ตั้ง Floor ใหม่ทุกต้นปีที่ 90% ของเงินที่มีตอนนั้น** ใช้ราคาปิดรายวัน ดอกเบี้ย 0% ไม่รวมปันผลและต้นทุน ตาม[กติกาทดสอบ S&P 500](#sp500-test) เส้นประคือ Buy & Hold; เส้นทึบคือ SLPI

<figure class="lesson-figure sp500-figure" id="sp500-slpi">
<picture>
<source media="(max-width: 600px)" srcset="assets/charts/sp500-slpi-mobile.svg">
<img src="assets/charts/sp500-slpi.svg" alt="กราฟเส้น SLPI เทียบ Buy &amp; Hold รายวัน ปี 2018–2025 เริ่ม 100 จบที่ 143.24 และ 256.04 หน่วยตามลำดับ ตั้ง Floor ใหม่ทุกปี" width="800" height="460">
</picture>
<figcaption>SLPI จบที่ 143.24 หน่วย เทียบ Buy &amp; Hold 256.04 หน่วย จากเงินเริ่ม 100 เท่ากัน กราฟใช้ 2,012 จุดรวมจุดเริ่มต้น แกนตั้งและช่วงเวลาเท่ากันทั้งสี่วิธี ที่มา: S&amp;P Dow Jones Indices ผ่าน FRED SP500; คำนวณตามกฎของบทเรียน</figcaption>
</figure>

**Drawdown ระหว่างทาง:** คำนวณจาก `(มูลค่าพอร์ต ÷ จุดสูงสุดสะสมของพอร์ต − 1) × 100` เส้นที่ 0% คืออยู่ที่จุดสูงสุด; −10% คือต่ำกว่าจุดสูงสุด 10% แต่ละเส้นใช้จุดสูงสุดของพอร์ตตัวเอง รวมเงินเริ่มต้น 100 และ **ไม่เริ่มวัดใหม่ต้นปี** แม้ Floor ของกลยุทธ์จะเริ่มรอบใหม่ก็ตาม

<figure class="lesson-figure sp500-figure sp500-drawdown" id="sp500-slpi-drawdown">
<picture>
<source media="(max-width: 600px)" srcset="assets/charts/sp500-slpi-drawdown-mobile.svg">
<img src="assets/charts/sp500-slpi-drawdown.svg" alt="กราฟ Drawdown รายวัน 2018–2025 ของ SLPI เทียบ Buy &amp; Hold จุดต่ำสุด −21.66% และ −33.92% ตามลำดับ วัดจากจุดสูงสุดของแต่ละพอร์ตต่อเนื่องข้ามปี" width="800" height="460">
</picture>
<figcaption>Drawdown ของ SLPI เทียบ Buy &amp; Hold จากเส้นทางเดียวกับกราฟมูลค่าด้านบน ใช้สเกล 0% ถึง −40% เหมือนกันทั้งสี่วิธี จุดต่ำสุดของเส้นเท่ากับ −21.66% และ −33.92% ตามลำดับ; Maximum drawdown รายงานเป็นขนาดบวก 21.66% และ 33.92% ไม่ใช่อัตรารายปี ที่มา: S&amp;P Dow Jones Indices ผ่าน FRED SP500; คำนวณตามกฎของบทเรียน</figcaption>
</figure>

เส้น SLPI กลายเป็นแนวราบหลังขาย เพราะถือเงินสดที่ดอกเบี้ย 0% และไม่กลับเข้าเสี่ยงจนจบปี เห็นชัดในปี 2020 และ 2025 ที่พลาดช่วงฟื้นตัว **เมื่อเริ่มปีใหม่ กลยุทธ์กลับเข้าถือสินทรัพย์เสี่ยง 100% และตั้งเกณฑ์ขายใหม่** จึงมีทั้งช่วงที่เส้นกลับเคลื่อนไหวและช่วงที่หยุดนิ่ง ไม่ใช่ขายครั้งเดียวแล้วถือเงินสดตลอดแปดปี

Maximum drawdown **ตลอดเส้นทางแปดปีนี้** เท่ากับ **21.66%** สำหรับ SLPI เทียบ **33.92%** ของ Buy & Hold ค่านี้วัดจากจุดสูงสุดก่อนหน้าโดยไม่เริ่มนับใหม่ต้นปี จึงต่างจากการเลือก Drawdown ที่มากที่สุดจากตารางการทดลองรายปี
...
<section id="sp500-test">

## 8.1. ลองกับ S&P 500: การป้องกันช่วยเมื่อไร และเสียโอกาสตรงไหน

ตัวอย่างสมมติช่วยให้เห็นกลไก ต่อไปเราจะนำกฎเดิมไปเดินบน **ราคาปิด S&P 500 จริง ปี 2018–2025** แล้วดูว่าเงินปลายทางกับความเสี่ยงระหว่างทางเปลี่ยนอย่างไร ใช้ครบทั้งแปดปี และแยกเล่าปี 2020, 2022 และ 2023 เพื่อให้เห็นตลาดร่วงแล้วฟื้น ตลาดขาลง และปีที่ดัชนีปรับขึ้น

ส่วนนี้เป็น **การทดสอบเพิ่มเติมของบทเรียน** ไม่ใช่ผลของวิทยานิพนธ์ปี 2018 และไม่ใช่ผลตอบแทนของกองทุนหรือบัญชีลงทุนจริง ราคาตลาดเป็นข้อมูลจริง แต่การซื้อขายและมูลค่า Option เป็นไปตามสมมติฐานด้านล่าง

กราฟเส้นแทรกไว้ในหัวข้อ [SLPI](#sp500-slpi), [CPPI](#sp500-cppi), [TIPP](#sp500-tipp) และ [Variable Multiplier](#sp500-variable-m) โดยทบผลรายปีเพื่อให้เห็นเส้นทางเงินตั้งต้น 100 ต่อเนื่องตลอดช่วง ส่วนตารางด้านล่างยังแยกเริ่ม 100 ใหม่ทุกปีเพื่อเทียบผลแต่ละปี ไม่มีกราฟของกลยุทธ์ Option ในชุดเปรียบเทียบนี้

### ข้อมูลและกติกาที่ใช้ร่วมกัน

ข้อมูลมาจาก **S&P Dow Jones Indices ผ่าน FRED ซีรีส์ SP500** เป็นราคาปิดรายวันของ Price Index **ไม่รวมเงินปันผล** ชุดที่ตรวจมี 2,325 จุด ตั้งแต่ 3 ตุลาคม 2016 ถึง 31 ธันวาคม 2025 ดึงข้อมูลวันที่ 19 กันยายน 2026 ใช้ช่วงก่อนปี 2018 เป็นข้อมูลตั้งต้นสำหรับประเมินความผันผวน [รายละเอียดข้อมูลต้นทาง](https://fred.stlouisfed.org/series/SP500)

- **เริ่มใหม่ทุกปีด้วย 100 หน่วยและ Floor 90** เริ่มที่ราคาปิดวันซื้อขายสุดท้ายของปีก่อน จบที่ราคาปิดวันซื้อขายสุดท้ายของปีนั้น ผลแต่ละปีเป็นการทดลองแยกกัน ไม่ใช่พอร์ตที่ถือยาวโดยไม่เริ่มใหม่
- **ดอกเบี้ยเงินปลอดความเสี่ยง 0%** ทุกวิธี เพื่อแยกผลของกฎป้องกันออกจากการเลือกสินทรัพย์ปลอดความเสี่ยง ไม่ใช่อัตราดอกเบี้ยจริงในแต่ละปี
- **ซื้อขายได้ที่ราคาปิดที่สังเกตพอดี** ถือเศษหน่วยได้ ไม่มีค่าธรรมเนียม สเปรด Slippage หรือภาษี ดัชนีเป็นตัวแทนสินทรัพย์เสี่ยง ไม่ใช่สินทรัพย์ที่ซื้อได้ตรง ๆ จึงยังไม่รวมต้นทุนหรือ Tracking error ของ ETF/Futures
- **ตรวจ Floor และ Drawdown จากราคาปิดรายวัน** จึงไม่เห็นการหลุดระหว่างวัน และ Stop Loss ไม่ได้สมมติว่าขายได้พอดีที่ 90
- **ข้อมูลสำหรับความผันผวนสิ้นสุดก่อนวันซื้อขายหนึ่งวันทำการ** ใช้ส่วนเบี่ยงเบนมาตรฐานแบบตัวอย่างของ Log return คูณ $\sqrt{252}$; ไม่ใช้ราคาหรือความผันผวนในอนาคต และไม่ได้ปรับพารามิเตอร์เพื่อให้ชนะในช่วงที่ทดสอบ

| วิธี | กฎที่นำมาทดสอบ |
|---|---|
| Buy & Hold | ถือสินทรัพย์เสี่ยง 100% ตลอดปี เป็นพอร์ตอ้างอิง |
| SLPI | เริ่มเสี่ยง 100% เมื่อมูลค่าที่ราคาปิดไม่เกิน 90 ขายที่ราคาปิดนั้น แล้วไม่กลับเข้าเสี่ยงจนจบปี |
| CPPI | $m=3$, Floor 90 คงที่ ปรับพอร์ตรายวัน; เงินเสี่ยงไม่ต่ำกว่าศูนย์หรือเกินมูลค่าพอร์ต |
| TIPP | $m=3$ เหมือน CPPI แต่ Floor ยกเป็น 90% ของจุดสูงสุดของพอร์ตที่สังเกตได้ และไม่ลดลงระหว่างปี |
| Variable m | Floor 90 คงที่ ใช้ $m_t=\min(6,\max(1,3\times0.20/\hat\sigma_t))$ โดย $\hat\sigma_t$ มาจาก 20 ผลตอบแทนก่อนวันซื้อขาย ปรับรายวันและจำกัดเงินเสี่ยงเหมือน CPPI |
| OBPI* | กันเงิน 90 และใช้เงิน 10 ซื้อ European Call ที่ $K=100$ บนดัชนีที่ปรับฐานเริ่ม 100 ถือจนครบปี; ราคา Call และมูลค่าระหว่างปีคำนวณด้วย Black–Scholes, $r=q=0$, ใช้ความผันผวนย้อนหลัง 252 ผลตอบแทนก่อนเริ่มปีและตรึงไว้จนหมดอายุ |

**OBPI\* ต้องอ่านแยก:** เราไม่มีประวัติราคา Option ที่ซื้อขายจริง จึงใช้ราคาตามแบบจำลองและใช้ Historical volatility แทน Implied volatility จำนวน Call ที่ซื้อได้จึงเป็นผลของสมมติฐานราคา ผลนี้ช่วยอธิบายกลไก แต่ยังใช้จัดอันดับว่า OBPI ลงทุนจริงดีกว่าวิธีอื่นไม่ได้ ราคา Option ที่ต่ำอาจทำให้แบบจำลองซื้อสิทธิรับขาขึ้นได้มาก แม้ไม่ได้กู้เงิน

### ผลตอบแทนปลายปี: ใครรักษาเงิน และใครรับขาขึ้นได้มากกว่า

ผลตอบแทนคำนวณจาก $(V_T/100-1)\times100$ ตารางแสดงครบทุกปีของช่วงทดสอบ ค่าติดลบหมายถึงจบปีต่ำกว่าเงินเริ่มต้น; ไม่ได้บอกว่าระหว่างปีเคยขาดทุนลึกเท่าไร

บนมือถือ เลื่อนตารางแนวนอนเพื่อดูครบทุกวิธี

<!-- sp500-return-table:start -->
| ปี | Buy & Hold | SLPI | CPPI | TIPP | Variable m | OBPI* |
|---|---:|---:|---:|---:|---:|---:|
| 2018 | -6.24% | -12.06% | -2.45% | -1.75% | -3.33% | -10.00% |
| 2019 | 28.88% | 28.88% | 10.41% | 7.46% | 11.40% | 32.51% |
| 2020 | 16.26% | -14.99% | 0.76% | 0.76% | 4.21% | 22.57% |
| 2021 | 26.89% | 26.89% | 9.40% | 6.91% | 12.95% | 9.50% |
| 2022 | -19.44% | -11.34% | -5.62% | -5.52% | -5.75% | -10.00% |
| 2023 | 24.23% | 24.23% | 8.22% | 6.26% | 10.94% | 15.25% |
| 2024 | 23.31% | 23.31% | 7.86% | 6.09% | 12.98% | 34.12% |
| 2025 | 16.39% | -13.73% | 4.23% | 3.63% | 3.67% | 22.47% |
<!-- sp500-return-table:end -->

*OBPI เป็นผลจากราคา Option ตามแบบจำลองตามเงื่อนไขข้างต้น ทุกคอลัมน์ยังไม่หักต้นทุนและไม่รวมเงินปันผล*

### ราคาปลายปีอย่างเดียวซ่อนอะไรไว้

ปี **2020** Buy & Hold จบปีบวก **16.26%** แต่ระหว่างปีมี Maximum drawdown **33.92%** ส่วน CPPI จบปีเพียง **0.76%** แลกกับ Drawdown **8.66%** นี่แสดงข้อแลกเปลี่ยนของการลดเงินเสี่ยงหลังตลาดลง: ลดผลกระทบระหว่างทาง แต่มีเงินเสี่ยงเหลือน้อยเมื่อราคาฟื้น

SLPI ในปีเดียวกันขายเมื่อวันที่ **9 มีนาคม 2020** หลังพอร์ตลดผ่าน Floor จนเหลือประมาณ **85.01 หน่วย** และจบปี **−14.99%** เพราะกฎไม่กลับเข้าตลาด ข้อดีคือหยุดรับขาลงหลังขาย แต่ข้อเสียมีทั้งการขายต่ำกว่าเกณฑ์และการพลาดการฟื้นตัว การเรียกเกณฑ์ว่า Stop Loss 10% จึงไม่ได้รับประกันว่าขาดทุนสูงสุดจะเท่ากับ 10%

ปี **2022** ดัชนีจบปี **−19.44%** ขณะที่ CPPI, TIPP และ Variable m ขาดทุนประมาณ **5.5–5.8%** กฎกันเงินจึงช่วยในตลาดขาลงปีนี้ ส่วนปี **2023** Buy & Hold ได้ **24.23%** เทียบ CPPI **8.22%** และ TIPP **6.26%** ทำให้เห็นราคาที่จ่ายผ่านโอกาสขาขึ้นที่น้อยลง

Variable m ไม่ได้ชนะ CPPI ทุกปี: ปี **2018** ได้ **−3.33%** และ Drawdown **9.08%** เทียบ CPPI **−2.45%** และ **6.16%**; ปี **2025** ผลตอบแทนก็ต่ำกว่า CPPI การเพิ่มตัวคูณเมื่อความผันผวนย้อนหลังต่ำอาจเพิ่มความเสี่ยงก่อนตลาดเปลี่ยน ส่วนการลดตัวคูณหลังความผันผวนเพิ่มอาจมาช้า เราจึงต้องดูทั้งตัวประมาณความเสี่ยงและเส้นทางพอร์ต

<details>
<summary>ดู Maximum drawdown ของทุกวิธีในแต่ละปี</summary>

Maximum drawdown คือการลดลงมากที่สุดจากจุดสูงสุดก่อนหน้าในปีนั้น รวมมูลค่าเริ่มต้น 100 เป็นจุดแรก แสดงขนาดการลดลงเป็นค่าบวก ไม่ใช่ผลตอบแทนติดลบ และไม่ได้ทำให้เป็นอัตรารายปี

<!-- sp500-drawdown-table:start -->
| ปี | Buy & Hold | SLPI | CPPI | TIPP | Variable m | OBPI* |
|---|---:|---:|---:|---:|---:|---:|
| 2018 | 19.78% | 19.78% | 6.16% | 5.02% | 9.08% | 28.61% |
| 2019 | 6.84% | 6.84% | 2.96% | 1.96% | 6.15% | 8.82% |
| 2020 | 33.92% | 18.89% | 8.66% | 7.67% | 7.00% | 14.99% |
| 2021 | 5.21% | 5.21% | 2.43% | 1.53% | 4.86% | 4.14% |
| 2022 | 25.43% | 11.91% | 6.50% | 6.39% | 6.40% | 10.55% |
| 2023 | 10.28% | 10.28% | 4.44% | 2.86% | 8.34% | 11.08% |
| 2024 | 8.49% | 8.49% | 3.72% | 2.40% | 7.00% | 14.22% |
| 2025 | 18.90% | 17.42% | 5.43% | 4.85% | 6.42% | 13.42% |
<!-- sp500-drawdown-table:end -->

สำหรับ OBPI* ตัวเลขระหว่างทางใช้ราคา Option จากแบบจำลองเดียวกับตอนเริ่มปี ผลจึงขึ้นกับวิธีประเมินมูลค่า ไม่ใช่ราคาที่ขาย Option ได้จริง ข้อสังเกตสำคัญคือการมี Floor 90 ณ ปลายทางไม่ได้จำกัด Drawdown จากจุดสูงสุดให้ไม่เกิน 10%

</details>

### สรุปทั้งแปดปี โดยแยกผลตอบแทนกับการคุ้มครอง

<!-- sp500-summary-table:start -->
| วิธี | ผลตอบแทนเฉลี่ยต่อปี | Drawdown ในปีที่รุนแรงที่สุด | ปีที่ต่ำกว่า Floor ณ ราคาปิด |
|---|---:|---:|---:|
| Buy & Hold | 13.78% | 33.92% | 4/8 |
| SLPI | 6.40% | 19.78% | 4/8 |
| CPPI | 4.10% | 8.66% | 0/8 |
| TIPP | 2.98% | 7.67% | 0/8 |
| Variable m | 5.88% | 9.08% | 0/8 |
| OBPI* | 14.55% | 28.61% | 0/8 |
<!-- sp500-summary-table:end -->

“เฉลี่ยต่อปี” คือค่าเฉลี่ยเลขคณิตของผลตอบแทนแปดการทดลอง ไม่ใช่ CAGR ของพอร์ตต่อเนื่อง ส่วน “Drawdown ในปีที่รุนแรงที่สุด” คือค่าสูงสุดในตารางรายปี ไม่ใช่ Drawdown ที่คำนวณข้ามปี Floor ของ TIPP ยกตามจุดสูงสุด; ของวิธีอื่นใช้ 90 คงที่ สำหรับ Buy & Hold การนับต่ำกว่า 90 มีไว้เทียบเกณฑ์เดียวกัน ไม่ได้หมายความว่าพอร์ตนี้มีกฎป้องกัน

**ไม่พบการหลุด Floor ในตัวอย่าง CPPI, TIPP และ Variable m ไม่ได้พิสูจน์ว่าปลอดภัยทุกเส้นทาง** ผลนี้มาจากราคาปิดและการปรับพอร์ตรายวันในช่วงที่เลือก หากราคาร่วงข้าม Cushion ก่อนซื้อขาย หรือมีต้นทุน การคุ้มครองอาจต่างออกไป ดูตัวอย่าง Gap risk ที่เราคำนวณไว้ก่อนหน้า

| วิธี | ข้อดีที่การทดสอบช่วยให้เห็น | ข้อเสียหรือเงื่อนไขที่ต้องแลก |
|---|---|---|
| SLPI | กฎง่าย และหยุดรับความเสี่ยงตลาดหลังขาย | ราคาอาจผ่านเกณฑ์ก่อนขาย; ปี 2020 และ 2025 พลาดการฟื้นตัวหลังออกจากตลาด |
| CPPI | ลดเงินเสี่ยงเมื่อเข้าใกล้ Floor; ในชุดนี้ Drawdown ต่ำกว่า Buy & Hold ทุกปี | รับขาขึ้นได้น้อย และต้องปรับพอร์ตแทบทุกวัน; ยังมี Gap risk |
| TIPP | ยกส่วนที่ต้องการปกป้องตามกำไร; ในชุดนี้ Drawdown รายปีไม่มากกว่า CPPI | กันกำไรไว้มากขึ้นทำให้เงินรับขาขึ้นลดลง; Floor ที่ยกแล้วไม่ลดตามตลาด |
| Variable m | เปิดทางให้ปรับการรับความเสี่ยงตามสภาพที่ประเมิน; บางปีรับขาขึ้นมากกว่า CPPI | ความผันผวนย้อนหลังอาจไม่ทันเหตุการณ์ และบางปีทั้งผลตอบแทนกับ Drawdown แย่กว่า CPPI |
| OBPI* | โครงสร้างเงินสำรองบวก Call ทำให้เห็น Floor ปลายทางชัด และยังรับขาขึ้น | Premium กำหนดจำนวน Call ที่ซื้อได้; ผลและ Drawdown ที่เห็นยังอิงราคาแบบจำลอง ต้องตรวจด้วยราคา Option จริงก่อนใช้ตัดสินเชิงลงทุน |

ต้นทุนยังเป็นจุดที่ต้องทดสอบต่อ: CPPI และ TIPP มีการปรับเงินเสี่ยงเฉลี่ยประมาณ **250 ครั้งต่อปี** ในชุดนี้ ส่วน Variable m ประมาณ **230 ครั้ง** ไม่รวมการจัดพอร์ตครั้งแรกและไม่ทำรายการเกินจำเป็นที่สิ้นปี จำนวนครั้งไม่ใช่จำนวนเงิน Turnover และไม่ใช่ค่าธรรมเนียม การหักต้นทุนจริงต้องคำนวณพอร์ตใหม่เพราะจะกระทบ Cushion และการซื้อขายรอบต่อไป

### ตรวจความไวของ OBPI ก่อนเชื่อผลที่ดูดี

ให้เส้นทาง S&P 500 ปี 2020, งบ 100 และเงินสำรอง 90 เหมือนเดิม เปลี่ยนเฉพาะความผันผวนที่ใช้ตั้งราคา Call ตั้งแต่เริ่มปีเป็น 15%, 20% และ 30% แล้วตรึงค่านั้นจนครบกำหนด ผลตอบแทน OBPI* เปลี่ยนเป็น **17.16%, 10.38% และ 3.62%** ตามลำดับ เพราะ Call แพงขึ้นทำให้เงิน 10 ซื้อสิทธิได้น้อยลง ค่านี้เป็นการทดสอบสมมติฐาน ไม่ใช่ชุด Implied volatility ที่สังเกตจากตลาด

จึงยังสรุปไม่ได้จากค่าเฉลี่ยในตารางว่า OBPI* “ดีที่สุด” ผลที่ดูดีอาจมาจากต้นทุน Option ที่แบบจำลองให้ต่ำกว่าตลาดจริง การเทียบเพื่อใช้งานจริงยังต้องเพิ่มราคา Option, เงินปันผล, ดอกเบี้ย, ค่าซื้อขาย และข้อจำกัดการส่งคำสั่งให้สอดคล้องกันทุกวิธี

### บทสรุปจากการทดสอบ

ถ้าถามเพียงว่าใครได้เงินมากที่สุด เราจะมองข้ามช่วงที่ต้องทนถือและโอกาสหลุดเป้าหมาย การทดสอบนี้ช่วยให้แยกสามคำถาม: **ต้องการปกป้องเงินต้นหรือกำไรที่เกิดขึ้นแล้ว, ยอมเสียโอกาสขาขึ้นเท่าไร, และกลไกจะทำงานทันเมื่อราคาลงหรือไม่** CPPI กับ TIPP ช่วยอธิบายการกันเงิน, Variable m เพิ่มความเสี่ยงจากการประมาณค่า, SLPI แสดงผลของการออกแล้วไม่กลับเข้า และ OBPI ทำให้เห็นว่าราคาของสิทธิสำคัญพอ ๆ กับรูป Payoff

อ่านผลโดยเริ่มจากเป้าหมายและข้อจำกัดของแต่ละวิธี แล้วตรวจเส้นทางที่ทำให้วิธีนั้นเสียเปรียบด้วย ไม่เลือกจากปีเดียวหรือค่าเฉลี่ยเพียงตัวเดียว

ดาวน์โหลด [ผลคำนวณรายปี CSV](downloads/sp500-results.csv), [สมมติฐานและผลแบบ JSON](downloads/sp500-results.json) และ [โค้ด Python สำหรับทดสอบซ้ำ](downloads/sp500-backtest.py) ได้ ข้อมูลราคาดิบไม่รวมในชุดเว็บ; ใช้ข้อมูลต้นทางที่มีสิทธิ์ใช้งานและจัดเป็นคอลัมน์ `date,close` แล้วรัน `python3 sp500-backtest.py sp500-input.csv results.json` ต้องมีประวัติก่อนเริ่มปี 2018 อย่างน้อย 253 จุดสำหรับค่าความผันผวน และครบถึงสิ้นปี 2025 ผลในหน้านี้ผูกกับช่วงข้อมูลและ SHA-256 ที่บันทึกไว้ใน JSON; ข้อมูลที่แก้ไขย้อนหลังอาจให้ผลต่างออกไป

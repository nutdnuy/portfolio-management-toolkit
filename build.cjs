...
async function build() {
  ...
  const {simulate} = await import('./src/math.mjs');
  ...
  const rows = simulate({scenario:'recovery'}).rows;
  const plot = (key, color, dash='') => `<path fill="none" stroke="${color}" stroke-width="2" ${dash ? `stroke-dasharray="${dash}"` : ''} d="${rows.map((r, i) => `${i ? 'L' : 'M'}${18 + i * 18},${139 - (r[key] - 65) * 1.35}`).join(' ')}"/>`;
  const cover = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 252 168" role="img"><title>CPPI and buy-and-hold on the same hypothetical recovery path</title><rect width="252" height="168" fill="white"/><text x="18" y="18" font-family="sans-serif" font-size="10" fill="#555">PORTFOLIO INSURANCE</text><path d="M18 146H234" stroke="#ddd"/>${plot('cppi','#6200ee')}${plot('buyHold','#00796b','5 3')}${plot('floor','#777','2 3')}<text x="18" y="161" font-family="sans-serif" font-size="8" fill="#555">12-month illustrative paths · initial value 100</text></svg>`;
  fs.writeFileSync(path.join(out, 'assets/portfolio-paths.svg'), cover);
...
<div class="book-layout"><aside id="book-sidebar" class="book-sidebar"><a href="index.html" class="cover-link" aria-label="กลับหน้า Welcome"><img class="book-cover" src="assets/portfolio-paths.svg" width="252" height="168" alt="ตัวอย่างสมมติ: CPPI และ Buy & Hold เริ่ม 100 หน่วยบนเส้นทางฟื้นตัว 12 เดือน เส้นประสีเทาคือ Floor 90"></a><a class="book-name" href="index.html">${escape(config.title)}</a><button class="search-trigger" id="search-button">${icon}<span>Search</span><kbd>⌘ K</kbd></button><nav class="book-nav" aria-label="สารบัญ">${nav}</nav>${contents}<div class="book-sidebar-footer"><a href="${escape(config.notebook)}" download>ดาวน์โหลด Notebook</a><a href="${page.file}.md" download>ไฟล์ Markdown หน้านี้</a><button id="theme-button">พื้นหลังมืด</button></div></aside>
<main class="book-main ${home ? 'welcome-main' : 'chapter'}" id="content"><div class="page-topline"><span>${escape(config.title)}</span><button id="print-button">พิมพ์หน้านี้</button></div>${page.body}<footer class="book-footer"><span>${escape(config.title)}</span><span>โดย ${escape(config.author)} · <a href="licenses/index.html">สิทธิ์การใช้งาน</a></span></footer></main></div>

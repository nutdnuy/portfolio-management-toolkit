...
let timer;const watcher=fs.watch(root,{recursive:true},(_,name)=>{if(!name||!(/^(src|content|assets|notebooks)\//.test(name)||/^(style\.css|book\.css|site\.config\.json|build\.cjs)$/.test(name)))return;clearTimeout(timer);timer=setTimeout(build,350);});

...
const pages = ['index', 'portfolio-insurance', 'lab', 'research', 'glossary'];
...
async function ready(page, name) {
  ...
  if (name === 'index') await page.locator('#home-demo [data-component="CountUp"]').waitFor();
  if (name === 'lab') await page.locator('#cppi-lab select').waitFor();
}
  ...
  try {
    for (const viewport of [{ width: 1440, height: 1000 }, { width: 390, height: 844 }, { width: 320, height: 844 }]) {
      for (const theme of ['dark', 'light']) {
        ...
        await context.addInitScript(theme => localStorage.setItem('pmt-theme', theme), theme);
        ...
        for (const name of pages) {
          ...
          if (viewport.width !== 320 && ['index', 'lab'].includes(name)) {
            const stem = name === 'index' ? 'home' : 'lab';
            const size = viewport.width === 1440 ? 'desktop' : 'mobile';
            await page.screenshot({ path: path.join(output, `${stem}-${size}${theme === 'light' ? '-light' : ''}.png`), fullPage: true, animations: 'disabled' });
          }
        }
        if (viewport.width < 1000) {
          await check(`Mobile navigation ${viewport.width}px ${theme}`, async () => {
            ...
            await page.getByRole('navigation', { name: 'Main navigation' }).getByRole('link', { name: 'Strategy lab' }).click();
    ...
    await check('Search: English, Thai, empty result, keyboard, Escape and return focus', async () => {
      ...
      assert.match(await page.locator('#search-status').textContent(), /No results/);
      await page.keyboard.press('Escape');
    ...
    await check('Saved scenarios recall values, remain keyboard accessible and export exact CSV', async () => {
      ...
      await lab.getByRole('button', { name: 'Save this run' }).click();
      ...
      await lab.getByRole('button', { name: 'Download CSV' }).click();
      ...
      for (let i = 0; i < 6; i++) await lab.getByRole('button', { name: 'Save this run' }).click();
    ...
    await check('Protective put payoff, wealth, cost and profit stay consistent', async () => {
      ...
      assert.match(await put.locator('.result-insight strong').textContent(), /Wealth 100\.00 − cost 106\.50 = -6\.50/);
    ...
    await check('Stepper all three stages and four real React Bits families', async () => {
      await ready(page, 'lab');
      const guide = page.locator('[data-component="Stepper"]');
      assert.equal(await guide.count(), 1);
      assert.equal(await guide.getByRole('button', { name: 'Back', exact: true }).isDisabled(), true);
      assert.match(await guide.locator('.rb-step-progress').textContent(), /Step 1 of 3/);
      await guide.getByRole('button', { name: 'Next', exact: true }).click();
      assert.match(await guide.locator('.rb-step-progress').textContent(), /Step 2 of 3/);
      assert.match(await guide.locator('.rb-step-panel').textContent(), /100 − 90 = 10/);
      await guide.getByRole('button', { name: 'Next', exact: true }).click();
      assert.match(await guide.locator('.rb-step-progress').textContent(), /Step 3 of 3/);
      assert.match(await guide.locator('.rb-step-panel').textContent(), /30 × 0.6 \+ 70 = 88/);
      assert.equal(await guide.getByRole('button', { name: 'Next', exact: true }).isDisabled(), true);
      await guide.getByRole('button', { name: 'Back', exact: true }).click();
      assert.match(await guide.locator('.rb-step-progress').textContent(), /Step 2 of 3/);
      await guide.locator('.rb-step-indicator').first().click();
      assert.match(await guide.locator('.rb-step-progress').textContent(), /Step 1 of 3/);
      await ready(page, 'index');
      assert.equal(await page.locator('[data-component="BlurText"]').count(), 1);
    ...
    await check('Reduced motion preserves headline, final numbers, new rows and stage meaning', async () => {
      await ready(reducedPage, 'index');
      const effects = await reducedPage.locator('.rb-blur-word').evaluateAll(words => words.map(el => { const s = getComputedStyle(el); return { opacity: s.opacity, filter: s.filter, transform: s.transform }; }));
      assert.ok(effects.length > 0);
      assert.ok(effects.every(s => s.opacity === '1' && s.filter === 'none' && s.transform === 'none'));
      await ready(reducedPage, 'lab');
      ...
      await reducedPage.getByRole('button', { name: 'Save this run' }).click();

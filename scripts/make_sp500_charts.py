"""Render four daily wealth comparisons from the locally held, pinned S&P 500 input.
...
def render(paths, method, mobile=False):
    box = layout(mobile)
    ...
    y = lambda value: bottom - value/300*(bottom-top)
    ...
    pieces = [f'<svg xmlns="http://www.w3.org/2000/svg" width="{w}" height="{h}" viewBox="0 0 {w} {h}" role="img" aria-labelledby="title desc">',
              ...
              '<metadata>Source: S&amp;P Dow Jones Indices via FRED SP500, retrieved 2026-09-19. Derived educational calculation. Calendar-date x axis; linear wealth y axis 0–300, common to all four charts. No smoothing or resampling. Annual rules and limitations: portfolio-insurance.html#sp500-test.</metadata>',
              ...
              text(18 if mobile else 28, 30, f'{method} vs Buy & Hold', 19 if mobile else 25),
              text(18 if mobile else 28, 54, 'S&P 500 · 2018–2025 · ตั้ง Floor ใหม่ทุกปี', 13 if mobile else 16, 'class="muted"')]
    ...
    for i, name in enumerate((BENCHMARK, method)):
        ...
        pieces.append(text(54, yy, f'{name}   {paths[name][-1][1]:.2f}', 15 if mobile else 18))
    pieces.append(text(left, top-13, 'มูลค่าพอร์ต · เริ่ม 100 หน่วย', 12 if mobile else 15, 'class="muted"'))
    for value in range(0, 301, 50):
        yy = y(value)
        pieces.append(f'<path d="M {left} {yy:.3f} H {right}" stroke="{"#bdbdbd" if value==100 else "#e5e5e5"}" stroke-width="1"/>')
    ...
    for name in (BENCHMARK, method):
        ...
        points = ' '.join(f'{x(date):.3f},{y(value):.3f}' for date,value in paths[name])
        ...
        pieces.append(f'<circle cx="{right}" cy="{y(paths[name][-1][1]):.3f}" r="3" fill="{color}"/>')
...
def generate(input_path):
    ...
    for slug, method in METHODS.items():
        for mobile in (False,True):
            filename=f'sp500-{slug}{"-mobile" if mobile else ""}.svg'
            svg=render(paths,method,mobile)
            (out/filename).write_text(svg,encoding='utf-8')
            metadata['files'][filename]={'sha256':hashlib.sha256(svg.encode()).hexdigest(), 'method':method, 'mobile':mobile, 'layout':layout(mobile)}
    (ROOT/'data/sp500-charts.json').write_text(json.dumps(metadata,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
    print('Generated four paired daily charts, desktop/mobile exports; 2012 observations per line. No raw quotes exported.')
